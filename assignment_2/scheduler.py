...
class Scheduler:
    ...
    def _get_priority(self,
        ...
        """
        return seq_group.priority, seq_group.first_seq.get_output_len()
    ...
    def _schedule_priority_preemption(
        ...
        if waiting_queue:
            ...
            # nothing is waiting, so move things between swapped and running
            swapped = deque(sorted(self.swapped, key=self._get_priority))
            seen_swapped = deque()

            while swapped:
                seq_group = swapped.popleft()

                if running_queue and self._get_priority(running_queue[-1]) <= self._get_priority(seq_group):
                    # request with least priority in running queue has higher priority than the highest swapped group, time to break
                    break
                ...
                seen_swapped.append(seq_group)

            swapped.extendleft(seen_swapped)
    ...
    def _schedule_cfs(self) -> SchedulerOutputs:
        ...
        # Always make space for prefills
        # if len(prefills.seq_groups
        #        ) == 0 and self.scheduler_config.policy == "priority":
        self._schedule_priority_preemption(budget)
        
        # Schedule prefill if there is any
